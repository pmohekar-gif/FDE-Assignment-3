...
cat <<'JSON'
{
  "systemMessage": "Warrant verification standard: run tests as `pytest -o addopts=` -- pyproject sets addopts=\"-q\", which hides the summary line, so a bare `pytest` cannot show you the counts. Baseline is 444 passed, 1 skipped (2026-09-10); any other numbers are a regression to explain, not to wave through. Full gate: `make check` (lint, typecheck, unit, integration, eval, build). Agent CLI argv check: `make verify-agent-cli`."
}
JSON
