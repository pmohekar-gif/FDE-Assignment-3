...
ASSIGNMENT_ISSUES: list[dict[str, object]] = [
    # Chirayu's completed development and PM/product ownership on feat/chirayu_warrant_features.
    {
        ...
        "body": (
            "Assigned to Chirayu. Completed from the R&D/product work: position Warrant as a safe"
            " agent-delegation control plane instead of a clone of Linear AI triage, duplicate "
            "detection, or semantic search. This ticket supports the pitch narrative and Naresh's"
            " product-rationale review."
        ),
    ...
    },
    {
        ...
        "body": (
            "Assigned to Chirayu. Completed on feat/chirayu_warrant_features via the Linear "
            "adapter work: map imported Linear issues into Warrant's issue table, preserve "
            "workspace boundaries, and let imported issues use triage, related issues, "
            "delegation, policy, warrant, and audit flows."
        ),
        "labels": ["assignee:chirayu", "linear", "adapter", "completed"],
        "paths": [
            "src/warrant/adapters/linear.py",
            "src/warrant/adapters/linear_dto.py",
            "tests/integration/test_linear_import.py",
        ],
        "demo_note": (
            "Shows Warrant can sit on top of a real tracker rather than only seeded issues."
        ),
    },
    {
        "key": "CHIR-1103",
        ...
        "body": (
            "Assigned to Chirayu. Completed in the feature branch: expose Linear logs and "
            "imported-ticket activity so the evaluator can see the adapter proof and operational "
            "trace from the Warrant dashboard."
        ),
        "labels": ["assignee:chirayu", "linear", "dashboard", "completed"],
        "paths": [
            "src/warrant/main.py",
            "src/warrant/templates/dashboard.html",
            "src/warrant/templates/linear_updates.html",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Chirayu. Completed on feat/chirayu_warrant_features: fetch GitHub pull "
            "request metadata and changed files, attach PR evidence to Warrant verification, and "
            "keep scope enforcement in deterministic Gate 1."
        ),
        "labels": ["assignee:chirayu", "github", "evidence", "completed"],
        "paths": [
            "src/warrant/adapters/github.py",
            "src/warrant/pr_review.py",
            "tests/integration/test_github_evidence.py",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Chirayu. Completed from the summarization work: generate a team-level "
            "update from deterministic Warrant facts without allowing generated prose to mutate "
            "issue state, policy, approvals, or warrants."
        ),
        "labels": ["assignee:chirayu", "summary", "manager-view", "completed"],
        "paths": [
            "src/warrant/service.py",
            "docs/features/TEAM_SUMMARY_EXECUTION.md",
            "tests/integration/test_team_summary_api.py",
        ],
    ...
    # Priyanka's completed development on main plus accountable sales/evidence work.
    {
        ...
        "body": (
            "Assigned to Priyanka. Completed on main: Slack events are signature-checked, "
            "deduplicated, mapped to Warrant users, and routed into the same policy/approval path"
            " for Q&A, status, and start coding commands."
        ),
        "labels": ["assignee:priyanka", "slack", "integration", "completed"],
        "paths": [
            "src/warrant/slack.py",
            "tests/integration/test_slack_adapter.py",
            "docs/features/AGENT_CODE_EXECUTION.md",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Priyanka. Completed on main: improve the server-rendered operator shell,"
            " dense queue groups, keyboard shortcuts, actor switcher, and demo navigation so "
            "Naresh can follow ALLOW, REQUIRE_APPROVAL, and DENY cases quickly."
        ),
        "labels": ["assignee:priyanka", "ui", "dashboard", "completed"],
        "paths": [
            "src/warrant/templates/dashboard.html",
            "src/warrant/templates/base.html",
            "src/warrant/static/app.css",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Priyanka. Completed on main and reflected in seed data: Priyanka is an "
            "admin demo actor, JWT/session auth ignores spoofed X-Actor-Id when enabled, and "
            "approvals record the authenticated authority in the audit chain."
        ),
        "labels": ["assignee:priyanka", "auth", "admin", "completed"],
        "paths": [
            "src/warrant/auth.py",
            "src/warrant/templates/login.html",
            "tests/integration/test_auth_gate.py",
        ],
        "demo_note": (
            "Shows Priyanka can approve protected work without pretending every user is an admin."
        ),
    },
    {
        "key": "PRI-2104",
        ...
        "body": (
            "Assigned to Priyanka. Accountable sales deliverable: collect consent-respecting "
            "interview notes, pricing reactions, GTM positioning, and "
            "hypothesis-to-evidence-to-decision traces. Do not fabricate users, metrics, quotes, "
            "or outcomes."
        ),
    ...
    # Gaurav's completed development from his branches, now represented as archived/handoff tickets.
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/features-first-phase: add SQLite FTS plus "
            "deterministic local-vector ranking, workspace filtering, result telemetry, and "
            "read-only semantic issue discovery."
        ),
        "labels": ["assignee:gaurav", "semantic-search", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/retrieval.py",
            "tests/integration/test_semantic_search_api.py",
            "docs/features/SEMANTIC_SEARCH_EXECUTION.md",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/features-first-phase: suggest related "
            "issues, expose active warrant overlaps, and feed those candidates into delegation "
            "risk without automatically changing issue state."
        ),
        "labels": ["assignee:gaurav", "related-issues", "retrieval", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/retrieval.py",
            "tests/integration/test_related_issues_api.py",
            "docs/features/RELATED_ISSUES_EXECUTION.md",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/features-first-phase: recommend team, "
            "priority, and labels with stale-revision protection and telemetry while keeping "
            "recommendations advisory until a human accepts them."
        ),
        "labels": ["assignee:gaurav", "triage", "advisory-ai", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/triage.py",
            "tests/integration/test_triage_recommendation_api.py",
            "docs/features/TRIAGE_RECOMMENDATIONS_EXECUTION.md",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/features-first-phase: generate delegation "
            "briefs from deterministic facts, include policy verdict provenance, cache by issue "
            "revision and prompt hash, and fall back to structured prose when the model is "
            "unavailable."
        ),
        "labels": ["assignee:gaurav", "delegation-brief", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/service.py",
            "tests/integration/test_delegation_brief_contract.py",
            "docs/features/DELEGATION_BRIEF_EXECUTION.md",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/ai-comments-code-intelligence: issue "
            "comments can invoke Warrant for grounded assistance while preserving rate limits, "
            "audit events, and non-authorising boundaries."
        ),
        "labels": ["assignee:gaurav", "ai-comments", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/main.py",
            "src/warrant/service.py",
            "tests/integration/test_ai_comments.py",
        ],
        "demo_note": (
            "Shows conversational assistance attached to a ticket without granting authority."
        ),
    },
    {
        "key": "GAU-3202",
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/ai-comments-code-intelligence: index the "
            "configured repository, answer code questions with citations, enforce path "
            "containment, skip secrets/binary/oversized files, and keep repository Q&A advisory."
        ),
        "labels": ["assignee:gaurav", "code-intelligence", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/repository.py",
            "src/warrant/templates/code.html",
            "tests/integration/test_agent_code_api.py",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav. Completed on gaurav/ai-comments-code-intelligence: users can "
            "create tickets through Warrant, duplicate submissions are idempotent, storage "
            "failures roll back, and comments can be added without starting authority work."
        ),
        "labels": ["assignee:gaurav", "new-ticket", "audit", "completed", "handoff:kriti"],
        "paths": [
            "src/warrant/templates/new_issue.html",
            "src/warrant/service.py",
            "tests/integration/test_issue_creation_api.py",
        ],
    ...
    },
    {
        ...
        "body": (
            "Assigned to Gaurav, handed to Kriti for validation. Completed yesterday: after "
            "ticket creation, route the user toward a Coding Session path, trigger the "
            "notification, and expose rerun coding session controls from the delegation view."
        ),
        "labels": ["assignee:gaurav", "coding-session", "notification", "handoff:kriti"],
        "paths": [
            "src/warrant/coding.py",
            "src/warrant/templates/delegation.html",
            "src/warrant/templates/coding_session.html",
        ],
        "demo_note": (
            "Shows the agent-execution lifecycle after Warrant has granted a scoped warrant."
        ),
    },
    ...
    # Kriti takeover tickets from today.
    {
        ...
        "body": (
            "Assigned to Kriti. Today the team walked Kriti through Slack, GitHub, Linear, "
            "internal architecture, docs, and a live Warrant demo. Acceptance: Kriti runs the "
            "demo herself and records setup or workflow feedback."
        ),
    ...
    },
    {
        ...
        "body": (
            "Assigned to Kriti. Validate the latest Gaurav handoff: ticket-created to "
            "coding-session routing, notification trigger, and rerun coding session option inside"
            " delegation detail. Improve anything that does not behave correctly."
        ),
    ...
    },
    {
        ...
        "body": (
            "Assigned to Kriti. Take over Gaurav's planned observability task: add structured "
            "logs around delegation creation, policy decision, warrant issuance, coding-session "
            "start/rerun, notifications, and verification failure without logging secrets or raw "
            "private data."
        ),
    ...
    # Scenario tickets deliberately cover Warrant's delegation outcomes and boundaries.
    {
        ...
        "body": (
            "A prior Warrant assignment task investigated checkout retry behaviour in "
            "services/billing/retry.py. It exists so PAY-4471 retrieves a real related candidate "
            "instead of relying on a random synthetic backlog."
        ),
    ...
    },
    {
        ...
        "body": (
            "Earlier web demo work updated reports empty-state wording in "
            "web/reports/EmptyState.tsx. It should appear as a related item and policy precedent "
            "when WEB-3001 or WEB-4519 are delegated."
        ),
    ...
    },
    {
        ...
        "body": (
            "Safe reversible web work for the Warrant demo. Acceptance: web/reports/Table.tsx "
            "remains in scope, the existing reports link is preserved, and the delegation should "
            "be eligible for ALLOW when requested by Chirayu as web/code owner."
        ),
    ...
    },
    {
        ...
        "body": (
            "Scenario ticket for Warrant's failure handling. If semantic retrieval is "
            "unavailable, Warrant should continue lexical-only, reduce completeness, and avoid "
            "unsafe automatic approval on insufficient context."
        ),
    ...
    },
    {
        ...
        "body": (
            "Scenario and submission ticket. Generate final evaluation artifacts for the "
            "submission ZIP, include metrics and generated evidence files, and exclude secrets, "
            "private client data, caches, virtual environments, node_modules, and build folders."
        ),
    ...
    },
    {
        ...
        "body": (
            "Track the Week 5 readiness checkpoint: validation result, remaining risk, pitch "
            "readiness, and constructive responses in the community discussion. Chirayu "
            "coordinates, Priyanka adds evidence, Kriti adds build status."
        ),
    ...
    # Naresh evaluator tasks.
    {
        ...
        "body": (
            "Assigned to Naresh as evaluator. Review that PAY-4471 requires approval, SEC-4502 is"
            " denied, WEB-4519 is allowed, and each result is supported by deterministic policy "
            "reasons, audit entries, and evidence contracts."
        ),
    ...
    },
    {
        ...
        "body": (
            "Assigned to Naresh as evaluator. Verify role evidence for Chirayu, Priyanka, and "
            "Kriti, while crediting Gaurav's completed work from his branches as archived "
            "development handoff rather than current team ownership."
        ),
        "labels": ["assignee:naresh", "role-evidence", "rubric"],
        "paths": ["README.md", "docs/DECISIONS.md"],
        "demo_note": (
            "Evaluator-facing checklist for current team ownership and contribution history."
        ),
    },
]
